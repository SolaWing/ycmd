...
LANGUAGE_SERVER_BIN = os.path.abspath( os.path.join(
  ...
  'third_party',
  'langserver-swift',
  '.build/release/langserver-swift',
  ) )
# DEBUG
LANGUAGE_SERVER_BIN = "/Users/wang/Library/Developer/Xcode/DerivedData/langserver-swift-bjyajjgydzhkfuctmlxspyzfravx/Build/Products/Debug/LanguageServer"
...
def ShouldEnableSwiftCompleter():
  _logger.info( 'Looking for Swift langserver' )
  if not os.path.exists(LANGUAGE_SERVER_BIN):
    _logger.warning( "Not enabling Swift lsp completion: Couldn't find Swift lsp bin" )
    return False
  return True
...
class SwiftCompleter( language_server_completer.LanguageServerCompleter ):
    ...
    return responses.BuildDebugInfoResponse(
      ...
      servers = [
        responses.DebugInfoServer(
          ...
          executable = LANGUAGE_SERVER_BIN,
          logfiles = [
            self._server_stderr,
  ...
  def _ServerIsRunning( self ):
    return self._server_handle # xcode debug fork subprocess
  ...
  def _StartServer( self, request_data, project_directory=None ):
    with self._server_state_mutex:
      ...
      _logger.debug( 'Starting swift-server with the following command: '
                     '{0}'.format( ' '.join( command ) ) )

      self._server_stderr = utils.CreateLogfile( 'SwiftLSP_stderr_' )
      ...
      try:
        ...
        _logger.error( 'SwiftLSP failed to start, or did not connect '
                       'successfully' )
        self._StopServer()
        return

    _logger.info( 'SwiftLSP Language Server started' )
  ...
  def _StopServer( self ):
    with self._server_state_mutex:
      ...
      try:
        ...
        _logger.info( 'SwiftLSP Language server stopped' )
      except Exception:
  ...
  def ConvertNotificationToMessage( self, request_data, notification ):
    if notification[ 'method' ] == 'language/status':
      ...
      return responses.BuildDisplayMessageResponse(
        'Initializing Swift completer: {0}'.format( message ) )

    return super().ConvertNotificationToMessage(
      request_data,
      notification )
