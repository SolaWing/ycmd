...
def ShouldEnableRustCompleter():
  _logger.info( 'Looking for Rust langserver' )
  if not os.path.exists(LANGUAGE_SERVER_BIN):
    _logger.warning( "Not enabling Rust lsp completion: Couldn't find Rust lsp bin" )
    return False
  return True
...
class RustCompleter( language_server_completer.LanguageServerCompleter ):
  ...
  def _StartServer( self, request_data, project_directory=None ):
    with self._server_state_mutex:
      ...
      _logger.info( 'Starting RustLSP Language Server...' )
      ...
      _logger.debug( 'Starting Rust-server with the following command: '
                     '{0}'.format( ' '.join( command ) ) )

      self._server_stderr = utils.CreateLogfile( 'RustLSP_stderr_' )
      ...
      try:
        ...
        _logger.error( 'RustLSP failed to start, or did not connect '
                       'successfully' )
        self._StopServer()
        return

    _logger.info( 'RustLSP Language Server started' )
  ...
  def _StopServer( self ):
    with self._server_state_mutex:
      _logger.info( 'Shutting down RustLSP...' )
      ...
      if not self._ServerIsRunning():
        _logger.info( 'RustLSP Language server not running' )
        self._CleanUp()
        return

      _logger.info( 'Stopping Rust server with PID {0}'.format(
      ...
      try:
        ...
        _logger.info( 'RustLSP Language server stopped' )
      except Exception:
        _logger.exception( 'Error while stopping RustLSP server' )
  ...
  def ConvertNotificationToMessage( self, request_data, notification ):
    if notification[ 'method' ] == 'language/status':
      ...
      return responses.BuildDisplayMessageResponse(
        'Initializing Rust completer: {0}'.format( message ) )
